...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    //verify if the omnibridge and tornadopool balance is updated
    const omniBridgeBalance = await token.balanceOf(omniBridge.address)
    expect(omniBridgeBalance).to.be.equal(0)
    const tornadoPoolBalance = await token.balanceOf(tornadoPool.address)
    expect(tornadoPoolBalance).to.be.equal(aliceDepositAmount)

    // Alice withdraws from L2
    const aliceWithdrawAmount = utils.parseEther('0.08')
    const aliceEthAddress = '0xA299DE74e1Bfb116Edb1813351A5e8ba9E1E7f86'
    ...
    //check if alice account has the updated balance
    //also verify the tornadopoolbalance
    const aliceBalance = await token.balanceOf(aliceEthAddress)
    expect(aliceBalance).to.be.equal(aliceWithdrawAmount)
    expect(tornadoPoolBalance).to.be.equal(utils.parseEther('0.1'))
    expect(omniBridgeBalance).to.be.equal(0)
  })
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    const aliceWithdrawAmount = utils.parseEther('0.13')
    const recipient = '0x6E5A623C92C9FcA068894A6ccEF284A1938186DE'
